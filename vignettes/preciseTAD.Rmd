...
The output of `preciseTAD::preciseTAD()` function is a list object with two elements: 1) the `preciseTAD` predicted boundary regions, and 2) the `preciseTAD` predicted boundary points (PTBP). From the results above, we see that `preciseTAD` yielded 12 predicted boundary points.
...
The Hela-S3-specific TFBS data can be downloaded at [ENCODE](http://www.encodeproject.org/chip-seq-matrix/?type=Experiment&replicates.library.biosample.donor.organism.scientific_name=Homo%20sapiens&assay_title=TF%20ChIP-seq&status=released) [??? Add more detailed link. Can you read BED files directly from ENCODE URLs?; !!!i don't think so, they require unzipping] or read from the [supplementary repository]( http://github.com/stilianoudakis/preciseTAD_supplementary/tree/master/data/bed/hela) [??? To use ExperimentHub;!!! looking into it]. They can be converted into a GRangesList.
