...
`preciseTAD` allows to use the pre-trained model to predict the precise location of domain boundaries in cell types without Hi-C data but with genome annotation data. Specifically, only cell type-specific ChIP-seq data (BED format) for CTCF, RAD21, SMC3, and ZNF143 transcription factor binding sites is required. 

These data need to be assembled in a GRanges object using the aforementioned `bedToGRangesList` function (i.e. creating another cell type-specific `tfbsList_filt` object). As an example, we consider Hela-S3 cell line. We can leverage our RF model trained on GM12878 ground truth boundaries (Arrowhead or Peakachu) to predict the boundaries for Hela-S3 on chr22.

The pre-trained model `tadModel` can be obtained by using `bedToGRangesList`, `extractBoundaries`, `createTADdata`, and `TADrandomForest` functions, as described. Alternatively, chromosome-specific pre-trained models can be downloaded from the [Google Drive folder](https://drive.google.com/drive/folders/10YGvuDxAuJtMkM7y5Pzntpkp1coomDzG). We recommend models pre-trained using GM12878 genome annotation data and Peakachu peaks. Note that each model is annotated with "CHRi_holdout", representing the chromosome that was held out of training, and thus to be used to predict boundaries. In the following example we use chr1-chr21 from GM12878 to predict TAD boundaries for chr22 from Hela-S3. Therefore, we opt to download [tadRF_CHR22_holdout_gm12878_arrowhead_gt.rds](https://1drv.ms/u/s!AhmbAyu-bORbgutoatqVlLzNMH_L1g?e=5AS27f). [??? Rename files. Swap to Peakachu model. Add direct download link; !!!swapped and added] 

```{r, eval=FALSE}
# Read in RF model built on chr1-chr21
tadModel <- readRDS("~/tadRF_CHR22_holdout_gm12878_arrowhead_gt.rds")
```

The Hela-S3-specific TFBS data can be downloaded at [ENCODE](https://www.encodeproject.org/chip-seq-matrix/?type=Experiment&replicates.library.biosample.donor.organism.scientific_name=Homo%20sapiens&assay_title=TF%20ChIP-seq&status=released) [??? Add more detailed link. Can you read BED files directly from ENCODE URLs?; !!!i don't think so, they require unzipping] or read from the [supplementary repository]( https://github.com/stilianoudakis/preciseTAD_supplementary/tree/master/data/bed/hela) [??? To use ExperimentHub;!!! looking into it]. They can be converted into a GRangesList.
