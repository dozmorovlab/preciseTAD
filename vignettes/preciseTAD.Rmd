...
Recall that our model is classifying TAD boundary **regions**, in that each prediction refers to a genomic bin of width 5000 base pairs. To more precisely predict TAD boundary coordinates at base pair resolution, we can leverage our model through the use of the `preciseTAD::preciseTAD` function. 

### Running preciseTAD

Suppose we want to use our model to precisely predict the TAD boundary coordinates for the 2 mb section of CHR22:25,500,000-27,500,000. To do so, we specify `chromCoords=list(25500000,27500000)`. Additionally, we set a threshold of 1.0 for constructing PTBRs using `threshold=1.0`. We also specify 100 random samples to be used in the partitioning of the pairwise genomic distances used by CLARA. For DBSCAN, we assign 5000 and 3 for the $\epsilon$ and *MinPts* parameters, respectively.
...
From the results above we see that *ARROWHEAD* yielded 18 called TAD boundary points (CTBP). However, `preciseTAD` yielded 13 precise TAD boundary points (PTBP). The corresponding predicted TAD boundary regions (PTBR) that flank each PTBP are also provided. 

### Using preciseTAD with Juicebox

Juicebox is an interface provided by Aiden Lab that allows for superimposing TAD boundary coordinates onto Hi-C contact maps. You must first select a Hi-C map. As an example, you can import the contact matrix for GM12878 derived by Rao et al. 2014 by choosing `File -> Open... -> Rao and Huntley et al. -> GM12878 -> in situ Mbol -> primary`. To format `preciseTAD` results to use in juicebox, set `juicer=FALSE`. The resulting PTBPs then need to be saved as a BED file and can be imported into juicebox using `Show Annotation Panel -> 2D Annotations -> Add Local -> myPTBPs.bed`.
