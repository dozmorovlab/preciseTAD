...
The output of `preciseTAD::preciseTAD()` function is a list object with two elements: 1) the `preciseTAD` predicted boundary regions, and 2) the `preciseTAD` predicted boundary points (PTBP). From the results above, we see that `preciseTAD` yielded `r length(pt$PTBP)` predicted boundary points.
...
The functionality provided by `preciseTAD` allows users to extend the model building and prediction processes described above to train and predict boundaries across cell types. This is beneficial when HiC data is unavailable for a particular cell type, likely due to high sequencing costs, yet ChIP-seq narrowpeak BED files are available. A particular example exists for Hela-S3, for which HiC data at 5 kb resolution is unavailable. Here, we can leverage our RF model built on GM12878 ground truth boundaries to predict boundaries for Hela-S3 using the corresponding cell type specific CTCF, RAD21, SMC3, and ZNF143 transcription factor binding sites. In the example below, we opt to use chr1-chr21 (omitting chr9) from GM12878 to predict TAD boundaries for chr22 from Hela-S3.

To do so, users build and train a boundary region prediction model on GM12878 as before using `bedToGRangesList`, `extractBoundaries`, `createTADdata`, and `TADrandomForest` functions. Next, users will need to recreate genomic annotations from Hela-S3 as a GRanges object using the aforementioned `bedToGRangesList` function (i.e. creating another cell type specific `tfbsList_filt` object). This, with the `tadModel[[1]]` object that is created is all that is necessary to predict boundaries on the other cell type using `preciseTAD`. 

We have streamlined this process for the user by providing pre-trained models built on GM12878 and K562, which are stored at https://drive.google.com/drive/folders/10YGvuDxAuJtMkM7y5Pzntpkp1coomDzG [??? temporary]. Each model is annotated with "CHRi_holdout", representing the chromosome that was held out of training, and thus to be used to predict boundaries. In the following example we use chr1-chr21 (omitting chr9) from GM12878 to predict TAD boundaries for chr22 from Hela-S3. Therefore, we opt to download "tadRF_gm12878_arrowhead_gt_CHR22_holdout.rds" [??? update name]. 

```{r, eval=FALSE}
# Read in RF model built on chr1-chr21
tadModel <- readRDS("~/tadRF_gm12878_arrowhead_gt_CHR22_holdout.rds")
```

The Hela-S3 specific TFBS can be downloaded at ENCODE or read in from the repository https://github.com/stilianoudakis/preciseTAD_supplementary/tree/master/data/bed/hela [??? temporary]. They can be converted into a GRangesList as such.

```{r, eval=FALSE}
# Establishing Hela-specific genomic annotations
# Reading in CTCF, RAD21, SMC3, and ZNF143 from repository and storing as list
ctcf <- as.data.frame(read_delim("https://raw.githubusercontent.com/stilianoudakis/preciseTAD_supplementary/master/data/bed/hela/ctcf.bed", delim = "\t", col_names = FALSE))
rad21 <- as.data.frame(read_delim("https://raw.githubusercontent.com/stilianoudakis/preciseTAD_supplementary/master/data/bed/hela/rad21.bed", delim = "\t", col_names = FALSE))
smc3 <- as.data.frame(read_delim("https://raw.githubusercontent.com/stilianoudakis/preciseTAD_supplementary/master/data/bed/hela/smc3.bed", delim = "\t", col_names = FALSE))
znf143 <- as.data.frame(read_delim("https://raw.githubusercontent.com/stilianoudakis/preciseTAD_supplementary/master/data/bed/hela/znf143.bed", delim = "\t", col_names = FALSE))
