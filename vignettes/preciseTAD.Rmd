...
Using built-in functions provided by `preciseTAD`, users can easily extend the model building and prediction processes described above to train and predict boundaries across cell types. This is beneficial when HiC data is unavailable for a particular cell type, likely due to high sequencing cost, yet ChIP-seq narrowpeak BED files are available. A particular example exists for Hela-S3, for which HiC data at 5 kb resolution is unavailable. Instead, we can leverage our RF model built on GM12878 ground truth boundaries to predict boundaries for Hela-S3 using the corresponding cell type specific CTCF, RAD21, SMC3, and ZNF143. To do so, users build and train a boundary region prediction model on GM12878 as before using `bedToGRangesList`, `extractBoundaries`, `createTADdata`, and `TADrandomForest` functions (See Section 3.1). Next, users will need to recreate genomic annotations from Hela-S3 as a GRanges object using the aforementioned `bedToGRangesList` function (i.e. creating another cell type specific `tfbsList_filt` object). They can be downloaded at ENCODE or read in from the repository https://github.com/stilianoudakis/preciseTAD_supplementary. This, with the `tadModel[[1]]` object that is created is all that is necessary to predict boundaries on the other cell type using `preciseTAD` (See Section 3.2). In the following example we use chr1-chr21 (omitting chr9) from GM12878 to predict TAD boundaries for chr22 from Hela-S3.
...
tadModel <- TADrandomForest(trainData = tadData[[1]],
                            ...
                            tuneParams = list(mtry = 2,
                                              ntree = 500,
                                              nodesize = ceiling(nrow(tadData[[1]])*.01)),
                            cvFolds = 3,
                            cvMetric = "Accuracy",
                            verbose = TRUE,
                            model = TRUE,
